# blockbusters-rental-manager DEV_NOTES.md

## define

- Build a CLI-only software solution to manage the Blockbuster games rental service.
...
- Employees operate the CLI on behalf of customers.
- Customers rent games, pay charges, and return games through an employee using the CLI.
- `return_status` uses named states: `rented`, `returned`, and `lost`.
- `game_id`, `membership_id`, and `rental_id` values are generated by the program and are not editable by users.
- `payment_method` stores labels only: `credit card` or `debit card`.
- Replacement cost defaults to `$40`.
